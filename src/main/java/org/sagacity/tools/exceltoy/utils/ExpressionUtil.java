...
public class ExpressionUtil {
	...
	protected static double calValue(String value1, String opt, String value2) throws Exception {
		try {
			...
			if (opt.equals("+")) {
				return bdValue1.add(bdValue2).doubleValue();
			} else if (opt.equals("-")) {
				return bdValue1.subtract(bdValue2).doubleValue();
			} else if (opt.equals("*")) {
			...
			e.printStackTrace();
			throw new Exception("数值 " + value1 + " 和数值 " + value2 + " 在进行" + opt + "运算时非法！");
		}
		throw new Exception("运算符" + opt + "非法");
	}
	...
	protected static String calFunction(String function, String value) throws Exception {
		...
		try {
			...
			throw new Exception("函数" + function + "�?" + value + "非法!");
		}
